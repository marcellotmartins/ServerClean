...
vRP.prepare("vRP/set_ldata","REPLACE INTO vrp_looting_data(dkey,dvalue) VALUES(@key,@value)")
vRP.prepare("vRP/get_srvdata","SELECT dvalue FROM vrp_srv_data WHERE dkey = @key")
vRP.prepare("vRP/get_ldata","SELECT dvalue FROM vrp_looting_data WHERE dkey = @key")
...
function vRP.setlData(key,value)
	vRP.execute("vRP/set_ldata",{ key = key, value = value })
end
...
function vRP.getlData(key, cbr)
	local rows = vRP.query("vRP/get_ldata",{ key = key })
	if #rows > 0 then
		return rows[1].dvalue
	else
		return ""
...
function vRP.dropPlayer(source)
	...
	if user_id then
		if user_id and source then
			PerformHttpRequest(webh.logSaida, function(err, text, headers) end, 'POST', json.encode({
				embeds = {
					{ 
			...
			if not vRP.isBanned(user_id) then
				...
				if vRP.isWhitelisted(user_id) then
					...
					if vRP.rusers[user_id] == nil then
						...
						if user_id and GetPlayerEndpoint(source) then
							PerformHttpRequest(webh.logEntrada, function(err, text, headers) end, 'POST', json.encode({
								embeds = {
									{ 
					...
					deferrals.done("Você não possui whitelist. https://discord.gg/gFvFbtSSZF [ ID: "..user_id.." ]")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("Você foi banido! [ Mais informações em: https://discord.gg/gFvFbtSSZF ] ")
				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
...
-- vRP.prepare("vRP/createChar","SELECT * FROM character_current WHERE cid = @user_id")
...
AddEventHandler("vRPcli:playerSpawned",function()
	...
	if user_id then
		...
		-- local haveChar = vRP.query("vRP/createChar",{ user_id = user_id})
		-- if haveChar[1] then
		-- 	TriggerClientEvent("dossantos-fristspawn",source,false)
		-- else
		-- 	TriggerClientEvent("dossantos-fristspawn",source,true)
		-- end
	end
end)
