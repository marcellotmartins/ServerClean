...
src = Tunnel.getInterface("elitestore",src)
...
local agachar = false
RegisterKeyMapping("agacharo","Agachar","keyboard","LCONTROL")
RegisterCommand("agacharo",function(source,args)
	local ped = PlayerPedId()
	if not IsPedInAnyVehicle(ped) then
		RequestAnimSet("move_ped_crouched")
		RequestAnimSet("move_ped_crouched_strafing")
		while not HasAnimSetLoaded("move_ped_crouched") and not HasAnimSetLoaded("move_ped_crouched_strafing") do
			Wait(1)
		end
			if agachar then
...
AddEventHandler("player:enterTrunk",function(source,args)
	TriggerServerEvent("player:enterTrunkServer")
end)
...
RegisterKeyMapping("+Empurando","Empurando","keyboard","M")
local empurandot = false
RegisterCommand("+Empurando",function(source,args)
	if Vehicle.Vehicle ~= nil then
		local ped = PlayerPedId()
		if GetEntityHealth(ped) > 100 and IsVehicleSeatFree(Vehicle.Vehicle,-1) and not IsEntityInAir(ped) and not IsPedBeingStunned(ped) and not IsEntityAttachedToEntity(ped,Vehicle.Vehicle) and not (GetEntityRoll(Vehicle.Vehicle) > 75.0 or GetEntityRoll(Vehicle.Vehicle) < -75.0) then
...
AddEventHandler("setalgemas",function()
	local ped = PlayerPedId()
	if GetEntityModel(ped) == GetHashKey("mp_m_freemode_01") then
		TriggerServerEvent('radio:outServers')
