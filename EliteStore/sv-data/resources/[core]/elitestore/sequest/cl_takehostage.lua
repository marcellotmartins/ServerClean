...
local hostageAllowedWeapons = { "WEAPON_PISTOL_MK2",
								"WEAPON_COMBATPISTOL",}
   ...
   --		TriggerEvent("Notify",source,"negado","Você precisa de uma FiveSeven com munição para fazer um refém sob a mira de uma arma!")
		   drawNativeNotification("Você precisa de uma FiveSeven com munição para fazer um refém sob a mira de uma arma!")
	   end
	   ...
	   if not holdingHostageInProgress and canTakeHostage then		
		   ...
		   if closestPlayer ~= -1 then
			   ...
			   drawNativeNotification("Ninguém por perto para tomar como refém!")
			   TriggertEvent("Notify",source,"negado","Ninguém por perto para tomar como refém!")
		   end 
	   end
   ...
   Citizen.CreateThread(function()
	   while true do 
		   if holdingHostage then
			   ...
			   DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Pressione [G] para liberar, [H] para matar")
