...
vRPN = {}
Tunnel.bindInterface("elitestore",vRPN)
Proxy.addInterface("elitestore",vRPN)

local idgens = Tools.newIDGenerator()
local cfg = module("elitestore","doors")
local timers = {}
local pick = {}
local blips = {}
...
AddEventHandler('vrpdoorsystem:open',function(id)
	...
	if vRP.hasPermission(user_id,cfg.list[id].perm) or vRP.hasPermission(user_id,"administrador.permissao") or vRP.hasPermission(user_id,"manager.permissao") then
	...
	if timers[id] == 0 or not timers[id] then
		...
		if cfg.list[id].other ~= nil then
			if vRP.getInventoryItemAmount(user_id,"lockpick") >= 1 and vRP.tryGetInventoryItem(user_id,"lockpick",1) then
				...
				local policia = vRP.getUsersByPermission(cfg.policiapermissao)
				...
				for k,v in pairs(policia) do
					...
					if player then
						async(function()
							...
							TriggerClientEvent('chatMessage',player,"COPOM",{64,64,255},".Alarme de estabelecimento disparando.")
