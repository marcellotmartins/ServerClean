	...
	$.post("http://vrp_crafting/requestCrafting", JSON.stringify({ craft: selectCraft }), (data) => {
		...
		for (let x = 1; x <= mySlots; x++) {
			...
			if (data.inventoryUser[slot] !== undefined) {
				...
				const item = `<div class="item populated" style="background-image: url('http://127.0.0.1/imageserver/vrp_itens1/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-amount="${v.amount}" data-slot="${slot}">
				<div class="top">
		...
		for (let x = 1; x <= inSlots; x++) {
			...
			if (nameList2[x - 1] !== undefined) {
				...
				const item = `<div class="item populated" style="background-image: url('http://127.0.0.1/imageserver/vrp_itens1/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-item-list="${list}" data-slot="${slot}">
				<div class="top">
