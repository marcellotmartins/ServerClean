	...
	$.post("http://vrp_inventory/requestMochila", JSON.stringify({}), (data) => {
		$(".myInfos").html(`
			...
			<div class="infosContent">
				...
				<span><s>EMPREGO:</s> ${data.infos[3]}</span>
		...
		for (let x = 1; x <= 100; x++) {
			...
			if (data.inventario[slot] !== undefined) {
				...
				const item = `<div class="item populated" style="background-image: url('http://127.0.0.1/imageserver/vrp_itens1/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v.amount}" data-peso="${v.peso}" data-item-key="${v.key}" data-name-key="${v.name}" data-slot="${slot}">
					<div class="top">
		...
		for (let x = 1; x <= 100; x++) {
			...
			if (nameList2[x - 1] !== undefined) {
				...
				const item = `<div class="item populated" style="background-image: url('http://127.0.0.1/imageserver/vrp_itens1/${v.index}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v.key}" data-name-key="${v.name}" data-id="${v.id}" data-grid="${v.grid}" data-amount="${v.amount}" data-peso="${v.peso}" data-slot="${slot}">
					<div class="top">
						<div class="itemWeight"></div>
						<div class="itemAmount">${formatarNumero(v.amount)}x   |   ${(v.peso * v.amount).toFixed(2)}</div>
					</div>
