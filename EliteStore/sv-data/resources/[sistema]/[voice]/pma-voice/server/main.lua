...
mutedPlayers = {}
function mutePlayer(target, state)
	local currentState = MumbleIsPlayerMuted(target)
	if currentState == state then return false end

	Player(target).state.muted = state
...
RegisterServerEvent('custom_pma:mutePlayer')
AddEventHandler('custom_pma:mutePlayer', function(state)
	mutePlayer(source, state)
end)
